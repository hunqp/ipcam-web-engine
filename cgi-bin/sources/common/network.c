 ...
 */
int Kiwi_DNS_GetAddress(uint8_t *u8Dns, char *stDns) {
    if (res_init() != 0) {
        return -1;
    }
    if (_res.nscount > 0) {
        ...
        if (u8Dns) {
            memcpy(u8Dns, &addr->sin_addr, 4);
        }
        if (stDns) {
            inet_ntop(AF_INET, &addr->sin_addr, stDns, INET_ADDRSTRLEN);
        }
        return 0;
    }
    return -2;
 ...
 */
int Kiwi_IP4_GetAddress(const char *interface, uint8_t *u8Ip, char *stIp) {
    bool boolean = false;
    ...
    for (p = list; (p != NULL) && (!boolean); p = p->ifa_next) {
        ...
        if ((strcmp(p->ifa_name, interface) == 0) && (p->ifa_addr->sa_family == AF_INET)) {
            ...
            /* IP4 Address */
            if (u8Ip) {
                memcpy(u8Ip, &sa->sin_addr, 4);
            }
            if (stIp) {
                inet_ntop(AF_INET, &sa->sin_addr, stIp, 16);
            }
            boolean = true;
        }
    }
 ...
 */
int Kiwi_MAC_GetAddress(const char *interface, uint8_t *u8Mac, char *stMac) {
    char chars[32] = { 0 };
    ...
    if (fp) {
        ...
        if (u8Mac) {
            for (int i = 0; i < 6; ++i) {
                u8Mac[i] = (uint8_t)values[i];
            }
        }
        if (stMac) {
            snprintf(stMac, 18, "%02X:%02X:%02X:%02X:%02X:%02X", values[0], values[1], values[2], values[3], values[4], values[5]);
        }
        return 0;
    }
    return -2;
 ...
 */
int Kiwi_GATEWAY_GetAddress(const char *interface, uint8_t *u8Gw, char *stGw, uint8_t *u8SubnetMask, char *stSubnetMask) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (dest == 0 && strcmp(iFace, interface) == 0) {
            ...
            */
            ...
            if (stGw) {
                snprintf(stGw, 16, "%d.%d.%d.%d", gw & 0xFF, (gw >> 8) & 0xFF, (gw >> 16) & 0xFF, (gw >> 24) & 0xFF);
            }
            ...
            */
            if (u8SubnetMask) {
                u8SubnetMask[0] = (mask & 0xFF);
                u8SubnetMask[1] = (mask >> 8) & 0xFF;
                u8SubnetMask[2] = (mask >> 16) & 0xFF;
                u8SubnetMask[3] = (mask >> 24) & 0xFF;
            }
            if (stSubnetMask) {
 ...
 */
int Kiwi_DNS_ResolveDomain(const char *domain, uint8_t *u8Ip, char *stIp) {
    int status;
    ...
    for (p = res; p != NULL; p = p->ai_next) {
        ...
        if (u8Ip) {
            memcpy(u8Ip, &ipv4->sin_addr, 4);
        }
        if (stIp) {
            inet_ntop(AF_INET, &ipv4->sin_addr, stIp, 16);
        }
        freeaddrinfo(res);
        return 0;
    }
...
bool Kiwi_WiFi_GetSSIDConnected(const char *wirelessInterface, char *ssid, size_t ssidLen) {
    if (!wirelessInterface || !ssid || ssidLen == 0) {
        return false;
    }

    char run[128] = {0};
    snprintf(run, sizeof(run), "wpa_cli -i %s status | grep '^ssid=' | cut -d= -f2-", wirelessInterface);
    FILE *fp = popen(run, "r");
    if (!fp) {
        return false;
    }
    if (!fgets(ssid, ssidLen, fp)) {
        pclose(fp);
        return false;
    }
    ssid[strcspn(ssid, "\r\n")] = '\0';
    pclose(fp);
    return (ssid[0] != '\0') ? true : false;
}
