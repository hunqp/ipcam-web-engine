 ...
 * @param u8Dns The IP4 address of the DNS server (in network byte order) - MUST BE 4 BYTES.
 * @param stDns The IP4 address of the DNS server (in network byte order) - MUST BE 4 BYTES.
 * @return 0 on success, -1 if res_init() fails, -2 if the DNS server address is not found.
 */
extern int Kiwi_DNS_GetAddress(uint8_t *u8Dns, char *stDns);
 ...
 */
extern int Kiwi_IP4_GetAddress(const char *interface, uint8_t *u8Ip, char *stIp); 
 ...
 */
extern int Kiwi_MAC_GetAddress(const char *interface, uint8_t *u8Mac, char *stMac);
 ...
 */
extern int Kiwi_GATEWAY_GetAddress(const char *interface, uint8_t *u8Gw, char *stGw, uint8_t *u8SubnetMask, char *stSubnetMask);
 ...
 */
extern int Kiwi_DNS_ResolveDomain(const char *domain, uint8_t *u8Ip, char *stIp);
